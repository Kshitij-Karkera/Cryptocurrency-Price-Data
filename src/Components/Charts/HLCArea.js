...
function HLCArea({ theme, data, interval, symbol }) {
    ...
    useEffect(() => {
        ...
        const domainPadding = halfRange * 0.20;

        let y = scaleLinear().domain([currentPrice - halfRange - domainPadding, currentPrice + halfRange + domainPadding]).range([height, 0]);

        const closeLineGenerator = line()
            .x(d => x(new Date(d[0])))
            .y(d => y(parseFloat(d[4])));

        const highAreaGenerator = area()
            .x(d => x(new Date(d[0])))
            .y0(d => y(parseFloat(d[4])))
            .y1(d => y(parseFloat(d[2])));

        const lowAreaGenerator = area()
            .x(d => x(new Date(d[0])))
            .y0(d => y(parseFloat(d[4])))
            .y1(d => y(parseFloat(d[3])));
        ...
        renderChartElements(zoomableContainer, x, y, data, closeLineGenerator, highAreaGenerator, lowAreaGenerator);
        ...
        const zoomBehavior = zoom()
            ...
            .on('zoom', (event) => {
                ...
                renderChartElements(zoomableContainer, newX, y, visibleData, closeLineGenerator, highAreaGenerator, lowAreaGenerator);
    ...
    function renderChartElements(container, xScale, yScale, data, closeLineGenerator, highAreaGenerator, lowAreaGenerator) {
        closeLineGenerator.x(d => xScale(new Date(d[0])));
        highAreaGenerator.x(d => xScale(new Date(d[0])));
        lowAreaGenerator.x(d => xScale(new Date(d[0])));

        container.selectAll('.line-path').remove();
        container.selectAll('.area-path').remove();

        container.append("path")
            .datum(data)
            .attr("fill", "green")
            .attr("opacity", 0.3)
            .attr("class", "area-path high-close")
            .attr("d", highAreaGenerator);

        container.append("path")
            .datum(data)
            .attr("fill", "red")
            .attr("opacity", 0.3)
            .attr("class", "area-path low-close")
            .attr("d", lowAreaGenerator);
